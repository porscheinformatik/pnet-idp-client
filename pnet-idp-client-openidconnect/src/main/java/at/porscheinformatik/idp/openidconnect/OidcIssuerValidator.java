...
public class OidcIssuerValidator implements OAuth2TokenValidator<Jwt>
{
    ...
    public OAuth2TokenValidatorResult validate(Jwt token)
    {
        ...
        if (issuer == null)
        {
            return OAuth2TokenValidatorResult
                .failure(invalidIdToken("The issuer is missing in the client registration metadata"));
        }

        if (!Objects.equals(issuer, token.getIssuer().toExternalForm()))
        {
            return OAuth2TokenValidatorResult
                .failure(invalidIdToken("The issuer does not match the client registration issuer"));
        }
    ...
    {
        return new OAuth2Error("invalid_id_token", message,
